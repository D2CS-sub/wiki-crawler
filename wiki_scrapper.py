...
def build_graph(draw: bool = True, save: bool = False, **kwargs):
    ...
    if len(languages) > 1:  # if there is more than one language.
        ...
        for vertex in G.nodes:  # go over the vertices.
            ...
            # get the translations of the page. if the language is not in the languages list, skip it.
            # handle errors with family wikipedia.
            langlinks = dict()
            ...
            request = site.simple_request(
                ...
                prop='langlinks',
                titles=vertex,
                lllimit='max'  # get all lang links
            )
            ...
            for page_id in pages:
                ...
                if 'langlinks' in page_info:
                    for langlink in page_info['langlinks']:
                        lang_code = langlink['lang']
                        title = langlink['*']
                        if lang_code in languages:
                            langlinks[lang_code] = title

            """for l_ in page.langlinks():
                try:
                    code = l_.site.lang  # get the code of the language.
                    title = l_.title
                    if code in languages:
                        langlinks[code] = title  # add the translation to the dictionary.
        ...
        for v in new_vertices:  # go over the new vertices.
            if len(v) == 0:
                continue  # skip the empty vertices.
            if add_content:
...
def get_section_links(page: pywikibot.Page, site: pywikibot.Site) -> List:
    ...
    try:
        ...
        print(f"Error extracting sections from page '{page.title()}': {e}")
        return []
